...
# -----------------------
# Fuzzy logic setup - DO NOT CHANGE
# -----------------------
...
def detect_cardiac_event(device, hr, avg_hr, std_hr, signal=None):
    """ML classification using 1D-CNN + GRU + Fuzzy logic"""
    if signal is None or len(signal) < 250:
        return "invalid"
    ...
    arrhythmia_score = float(pred[2])  # arrhythmia class probability
...
def forward_to_cloud(payload):
    ...
    if not success:
        print("[GATEWAY] ⚠ Cloud link unstable (simulated loss)")
        return
    try:
...
# -----------------------
# Data reception and processing - DO NOT CHANGE
# -----------------------
def receive_from_router(payload_bytes):
    ...
    if key in seen:
        print("[GATEWAY] Duplicate packet ignored:", key)
        return
    seen.add(key)
    ...
    preprocessed_ecg = data.get("preprocessed_ecg")  # ECG segment from router
    ...
    # ML + Fuzzy classification
    event_type = detect_cardiac_event(device, hr, avg_hr, std_hr, signal=preprocessed_ecg)
    alert_flag = event_type in ["critical", "moderate", "bradycardia", "tachycardia", "arrhythmia"]

    now = time.time()
    if alert_flag and (now - last_alert_time[device] > 10):
        print(f"[GATEWAY ALERT] ⚠ {device.upper()} → {event_type.upper()} DETECTED | HR={hr:.1f}, AVG={avg_hr:.1f}")
    ...
    c_local.execute("""
        INSERT INTO readings (device, seq, hr, avg_hr, temp, ts, priority, num_peaks, num_samples)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
    """, (
        device, data.get("seq"), hr, avg_hr, data.get("temp"),
        ts, priority, data.get("num_peaks"), data.get("num_samples")
    ))
    conn_local.commit()
    conn_local.close()

    hr_str = f"{hr:.1f}" if hr is not None else "0.0"
    avg_hr_str = f"{avg_hr:.1f}" if avg_hr is not None else "0.0"
...
class SimpleHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        if self.path.startswith("/last"):
            c.execute("""
                SELECT device, seq, hr, avg_hr, temp, ts, priority, num_peaks, num_samples
